...
pub mod wasmredis {
    ...
    pub mod host {
        ...
        pub mod store {
            ...
            impl Kvstore {
                #[allow(unused_unsafe, clippy::all)]
                pub fn delete(&self, key: &str) -> Result<_rt::Vec<u8>, Error> {
                    unsafe {
                        #[repr(align(4))]
                        struct RetArea([::core::mem::MaybeUninit<u8>; 16]);
                        let mut ret_area = RetArea(
            ...
            pub mod run {
                ...
                macro_rules! __export_wasmredis_host_run_cabi {
                    ($ty:ident with_types_in $($path_to_types:tt)*) => {
                        const _ : () = { #[export_name = "wasmredis:host/run#run"] unsafe
...
macro_rules! __export_redis_store_impl {
    ...
    ($ty:ident with_types_in $($path_to_types_root:tt)*) => {
        $($path_to_types_root)*::
        exports::wasmredis::host::run::__export_wasmredis_host_run_cabi!($ty
        with_types_in $($path_to_types_root)*:: exports::wasmredis::host::run);
    };
}
#[doc(inline)]
...
\x07\x01j\0\x01\x02\x01@\x03\x04self\x03\x03keys\x05value\x04\0\x08\x04\0\x13[me\
thod]kvstore.set\x01\x09\x01j\x01\x04\x01\x02\x01@\x02\x04self\x03\x03keys\0\x0a\
